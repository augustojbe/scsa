...
├── entrypoint-app.sh          # wait_for_db → migrate advisory lock → collectstatic → gunicorn/runserver
├── entrypoint-celery.sh       # wait_for_db → celery worker/beat
├── docker-compose.yml         # Ambiente local (app, postgres, celery_worker, celery_beat, rabbitmq, redis, traefik)
├── docker-stack.yml           # Produção Docker Swarm (Sprint 30)
├── traefik/
│   └── traefik.yml            # Traefik produção (DNS-01, TLS wildcard)
├── requirements.txt
├── manage.py
├── .env                       # gitignored
...
`traefik/traefik.yml` (resumo das diretivas obrigatórias):
...
```bash
docker build -t ghcr.io/pycodebr/scsi_v1:latest -f Dockerfile .
...
- [x] Criar `Dockerfile` (na raiz).
- [x] Criar `entrypoint-app.sh` (`wait_for_db` → migrate advisory lock → collectstatic --clear).
- [x] Criar `entrypoint-celery.sh` (`wait_for_db` apenas).
- [x] Criar `docker-compose.yml` (na raiz) com app, postgres, celery_worker, celery_beat, rabbitmq, redis, traefik.
- [x] Adicionar healthchecks locais.
- [x] Validar `docker compose up` e app respondendo.

### Sprint 3 — Configuração Django Base

- [x] Criar app `base` com `BaseModel` (`created_at`, `updated_at`), mixins, utils, constants.
- [x] Criar comando `wait_for_db`.
- [x] Criar view/rota `/health/` (sem banco, sem auth, retorna 200).
- [x] Configurar WhiteNoise `CompressedStaticFilesStorage`.
- [x] Configurar templates/base.html com herança e design system.

### Sprint 4 — Usuário Customizado com Login por Email

- [x] Criar custom user model em `core` (`email` como `USERNAME_FIELD`).
- [x] Configurar `AUTH_USER_MODEL`.
- [x] Criar fluxo de login por email (CBVs nativas do Django).
- [x] Criar recuperação de senha por email (templates nativos).
- [x] Configurar email backend via `.env`.

### Sprint 5 — Multi-Tenant Base

- [x] Criar model `Brokerage` em `core`.
- [x] Criar `TenantManager` e `BaseTenantModel` em `base`.
- [x] Criar middleware de tenant (`request.brokerage`).
- [x] Vincular `User` à `Brokerage`.
- [x] Garantir que `User.brokerage` é obrigatório (exceto superuser).

### Sprint 6 — Corretoras e Permissões

- [x] Criar app `accounts` com onboarding (corretora + usuário owner).
- [x] Cadastro de corretora com CNPJ + Razão Social obrigatórios.
- [x] Escolha de plano (free).
- [x] Definir roles/permissions (owner/admin, agent, producer, operational).
- [x] Mixins de permissão por role + tenant.

### Sprint 7 — Clientes

- [x] Criar app `clients` com `Client` (tenant).
- [x] CRUD de clientes (CBVs).
- [x] Tela de detalhe do cliente.
- [x] Campo `ai_summary` (placeholder para Sprint 23).
- [x] Anexos de cliente (Sprint 14).

### Sprint 8 — Seguradoras e Ramos

- [x] Criar app `insurers` com `Insurer` e `Branch`.
- [x] CRUD de seguradoras e ramos.
- [x] Relacionar ramos com propostas/apólices.

### Sprint 9 — Itens Cobertos e Coberturas

- [x] Criar `CoveredItem` e `Coverage` (app `policies`).
- [x] CRUD de coberturas.
- [x] CRUD de itens cobertos com atributos flexíveis (JSONField).

### Sprint 10 — Propostas

- [x] Criar `Proposal` (tenant) com cliente, seguradora, ramo, coberturas, itens cobertos, prêmio, status.
- [x] CRUD de propostas (CBVs).
- [x] Anexos de proposta (Sprint 14).
- [x] Campo `ai_summary`.

### Sprint 11 — Apólices

- [x] Criar `Policy` (tenant) com número, vigência, prêmio, status, coberturas, itens cobertos.
- [x] CRUD de apólices.
- [x] Anexos de apólice (Sprint 14).
- [x] Campo `ai_summary`.

### Sprint 12 — Geração de Apólice a partir da Proposta

- [x] Botão "Gerar apólice" na proposta.
- [x] Lógica que cria `Policy` a partir dos dados da proposta.
- [x] Marcar proposta como convertida.

### Sprint 13 — Sinistros

- [x] Criar app `claims` com `Claim` (tenant) vinculado a apólice + item coberto.
- [x] CRUD de sinistros.
- [x] Anexos de sinistro (Sprint 14).
- [x] Campo `ai_summary`.

### Sprint 14 — Anexos Privados

- [x] Implementar anexos em clientes, propostas, apólices, sinistros. *(clientes e propostas concluídos; apólices e sinistros pendem das Sprints 11/13)*
- [x] Criar view segura de download (valida auth + brokerage + permissão).
- [x] Configurar `MEDIA_ROOT` em volume.
- [x] 404 para cross-tenant.

### Sprint 15 — CRM Grid

- [x] Criar app `crm` com `Pipeline`, `PipelineStage`, `Deal`.
- [x] Visão grid de negociações com filtros.
- [x] CRUD de negociações.
- [x] Personalização de etapas (nome/cor/ordem).

### Sprint 16 — CRM Kanban

- [x] Visão kanban com colunas = etapas.
- [x] Drag-and-drop de cards entre etapas.
- [x] Atualizar `stage` ao mover (PATCH).
- [x] Campo `ai_summary` na negociação.

### Sprint 17 — Renovações

- [x] Criar app `renewals` com `Renewal` (tenant) vinculado a apólice.
- [x] Controle de vencimentos e status.
- [x] Alertas/relatórios de renovação.

### Sprint 18 — Endossos

- [x] Criar `Endorsement` vinculado a apólice.
- [x] CRUD de endossos.

### Sprint 19 — Agentes e Produtores

- [x] Criar app `agents` com `Agent` e `Producer`.
- [x] Hierarquia: agente → produtores; produtor pode ser direto da corretora.
- [x] CRUD de agentes e produtores.

### Sprint 20 — Comissões

- [x] Criar app `commissions` com `Commission`.
- [x] Cálculo de repasses (corretora, agente, produtor).
- [x] Relatórios de comissões (integrar com Sprint 21).

### Sprint 21 — Relatórios

- [x] Criar app `reports`.
- [x] Menu e tela dedicada a relatórios.
- [x] Exportação PDF (ReportLab/PyPDF).
- [x] Exportação CSV.
- [x] Relatórios: clientes, seguros, seguradoras, apólices, propostas, sinistros, renovações, comissões, carteira.

### Sprint 22 — Dashboard

- [x] Criar app `dashboard`.
- [x] Métricas de carteira, seguros, seguradoras, valores.
- [x] Gráficos diversos.
- [x] Funil de negociações/leads com níveis.
- [x] Cache em Redis quando aplicável.

### Sprint 23 — IA para Resumos

- [x] Criar app `ai` com agentes LangChain/LangGraph.
- [x] Configurar OpenAI (GPT-5.5-mini) via `.env`.
- [x] Implementar tools de leitura filtradas por tenant.
- [x] Tasks Celery: `summarize_client/policy/claim/proposal/deal`.
- [x] Salvar resumo em `ai_summary`.
- [x] Botão "Resumir com IA" com loading + aviso.

### Sprint 24 — Chat com IA

- [x] Models `ChatSession` e `ChatMessage`.
- [x] Tela de chat no menu lateral.
- [x] Sessões salvas por usuário.
- [x] Resposta em stream (SSE).
- [x] Resposta em Markdown renderizado em HTML (sanitizado).

### Sprint 25 — Celery, RabbitMQ e Redis

- [x] Configurar Celery com RabbitMQ (broker) e Redis (result backend + cache).
- [x] Configurar `celery_beat` se houver agendamentos.
- [x] Instalar `dj-celery-panel` no admin.
- [x] Garantir que tasks de IA não bloqueiam a interface.

### Sprint 26 — Notificações Internas

- [x] Model `Notification` (tenant + user).
- [x] Criar notificação ao concluir task de IA (resumo pronto).
- [x] Exibir notificações na interface (toast/badge).
- [x] Marcar como lida.

### Sprint 27 — Dados Fake

- [x] Criar command `load_fake_data`.
- [x] Carga de corretora(s), usuários, clientes, seguradoras, ramos, coberturas, propostas, apólices, itens cobertos, sinistros, renovações, endossos, agentes, produtores, comissões, pipeline.
- [x] Usar diversas datas diferentes para demonstração realista.

### Sprint 28 — Admin Django

- [x] Registrar todas as entidades no admin.
- [x] Adicionar `search_fields` e `list_filter`.
- [x] Sobrescrever `get_queryset` para filtrar por tenant (exceto superuser).
- [x] Validar `brokerage` em save.
- [x] Integrar `dj-celery-panel`.

### Sprint 29 — MKDocs

- [x] Configurar MKDocs em `docs/`.
- [x] Habilitar suporte a Mermaid.
- [x] Documentar instalação, arquitetura, deploy e uso.
- [x] Servir documentação online.

### Sprint 30 — Docker Swarm

- [x] Criar `docker-stack.yml` com serviços, volumes, redes, restart_policy, resources, update_config.
- [x] Definir redes `traefik_public`, `scsi_v1_internal`, `scsi_v1_egress`.
- [x] Volumes nomeados (postgres, redis, rabbitmq, media, staticfiles, certs).
- [x] Healthchecks em todos os serviços.
- [x] Entrypoints com `wait_for_db` + advisory lock + collectstatic --clear.

### Sprint 31 — Traefik e TLS Wildcard

- [x] Configurar Traefik no stack.
- [x] Configurar DNS-01 com Cloudflare.
- [x] Secret `CLOUDFLARE_DNS_API_TOKEN`; `CF_DNS_API_TOKEN_FILE`.
- [x] Certificado wildcard `scsi.digital` e `*.scsi.digital`.
- [x] `forwardedHeaders.trustedIPs` (Cloudflare).
- [x] Redirect HTTP → HTTPS.

### Sprint 32 — Scripts de Deploy e Backup

- [x] Criar `scripts/deploy.sh` (parser seguro, validações, build, push, stack deploy, rollout, `--skip-build`).
- [x] Criar `scripts/backup.sh` (pg_dump + media + rotação por tempo + cron-ready).
- [x] Validar scripts na VPS.

### Sprint 33 — Hardening e Revisão Final

- [x] Revisar `DEBUG=False` em produção.
- [x] Revisar `ALLOWED_HOSTS` e `CSRF_TRUSTED_ORIGINS`.
- [x] Revisar isolamento multi-tenant em todas as queries/views.
- [x] Revisar proteção de media.
- [x] Revisar secrets (Docker Secrets, nada versionado).
- [x] Revisar redes Docker (celery fora da traefik_public).
- [x] Revisar healthchecks e restart policies.
- [x] Revisar resources limits/reservations.
- [x] Revisar sanitização de Markdown.
- [x] Rodar checklist final de qualidade (seção 40).

---

## 40. Checklist Final de Qualidade

- [x] Python > 3.13 e Django > 6.0.
- [x] Único `settings.py` com `django-environ`.
- [x] `.env` gitignored; produção e desenvolvimento separados.
- [x] Login por email (custom user).
- [x] Recuperação de senha por email nativa.
- [x] Timezone `America/Sao_Paulo`; `LANGUAGE_CODE=pt-br`.
- [x] Código em inglês; interface em português brasileiro.
- [x] Apps Django na raiz; `core` e `base` presentes.
- [x] Entidades separadas em apps por domínio.
- [x] Todo model com `created_at` e `updated_at`.
- [x] Signals em `signals.py` da app (quando usados).
- [x] Multi-tenant compartilhado com campo `brokerage`.
- [x] Sem schemas/bancos separados por tenant.
- [x] Middleware de tenant + `TenantManager`.
- [x] Admin filtra por tenant (exceto superuser).
- [x] Media privada via view segura (auth + brokerage + permissão).
- [x] Cross-tenant retorna 404 em download de anexos.
- [x] IA com LangChain > 1.0 + LangGraph; modelo GPT-5.5-mini.
- [x] Tools de IA sempre filtram por `brokerage_id` do usuário.
- [x] Resumos salvos em `ai_summary`.
- [x] Chat com IA: sessões, stream, Markdown sanitizado.
- [x] Celery + RabbitMQ (broker) + Redis (backend/cache).
- [x] `dj-celery-panel` no admin.
- [x] Tarefas de IA não bloqueiam a interface (loading + notificação).
- [x] Notificações internas ao concluir tasks.
- [x] Proposta gera apólice com botão dedicado.
- [x] Sinistro vinculado a apólice + item coberto.
- [x] CRM grid + kanban com etapas personalizáveis (nome/cor/ordem).
- [x] Funil de negociações no dashboard.
- [x] Relatórios PDF (ReportLab/PyPDF) + CSV.
- [x] Landing page com apenas plano free habilitado.
- [x] Admin com filtros e buscas.
- [x] Command `load_fake_data` com datas variadas.
- [x] MKDocs com Mermaid em `docs/`.
- [x] Design system referenciado em `design_system/design-system.html`.
- [x] `/health/` retorna 200 sem banco e sem auth.
- [x] Healthchecks em todos os serviços.
- [x] `wait_for_db` nos entrypoints.
- [x] Migrations com advisory lock no entrypoint do app.
- [x] `collectstatic --clear` no entrypoint do app.
- [x] Celery não roda migrations nem collectstatic.
- [x] Redes: `traefik_public` (external), `scsi_v1_internal` (internal), `scsi_v1_egress` (overlay).
- [x] `app` em `traefik_public` + `scsi_v1_internal`.
- [x] `db`, `redis`, `rabbitmq` apenas em `scsi_v1_internal`.
- [x] `celery_worker`/`celery_beat` em `scsi_v1_internal` + `scsi_v1_egress` (nunca em `traefik_public`).
- [x] Volumes nomeados: postgres, redis, rabbitmq, media, staticfiles, certs.
- [x] Traefik com DNS-01 (Cloudflare); wildcard `scsi.digital` + `*.scsi.digital`.
- [x] Secret `CLOUDFLARE_DNS_API_TOKEN`; `CF_DNS_API_TOKEN_FILE`.
- [x] Sem `tlschallenge` + `dnschallenge` no mesmo resolver.
- [x] `forwardedHeaders.trustedIPs` (Cloudflare).
- [x] Redirect HTTP → HTTPS.
- [x] `SECURE_PROXY_SSL_HEADER=('HTTP_X_FORWARDED_PROTO','https')`.
- [x] `/health/` em `SECURE_REDIRECT_EXEMPT`.
- [x] `ALLOWED_HOSTS` só hostnames (sem esquema); `localhost`/`127.0.0.1` presentes.
- [x] `CSRF_TRUSTED_ORIGINS` com `https` e wildcard.
- [x] `restart_policy` (on-failure) em todos os serviços.
- [x] `resources.limits`/`reservations` em todos os serviços.
- [x] `update_config` do app: `start-first` + `failure_action: rollback`.
- [x] Imagem no GHCR `ghcr.io/pycodebr/scsi_v1`.
- [x] Deploy com `docker stack deploy --with-registry-auth`.
- [x] `scripts/deploy.sh` com parser seguro, validações, `--skip-build`.
- [x] `scripts/backup.sh` com pg_dump + media + rotação por tempo.
- [x] Segredos não versionados.
- [x] Sem testes automatizados (conforme requisito explícito).
- [x] Código simples, legível, PEP8, aspas simples.
- [x] Preferência por Class Based Views e recursos nativos do Django.
